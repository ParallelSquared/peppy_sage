    ...
    InfoReader: Reader<FrameInfo> + IndexedReader<FrameInfo> + Sync + Send,
{
    ...
    pub fn get_peaks_from_frame(&self, index: usize) -> TimsResult<Vec<Peak>> {
        // Decode once: the ms_level/index lookup below needs the same frame the
        // transpose does, and decoding it twice doubles the decompression cost.
        let frame = self.frame_reader.get_frame(index).map_err(|e| {
            TimsError::new(format!("Failed to read frame {index}: {e}"))
        })?;
        let peaks = if frame.info().ms_level() == MSLevel::MS1 {
            self.centroider_ms1
                .centroid(TofSlices::new(&frame), frame.index())
                .collect()
...
/// Yields a frame's ions grouped by tof index, in ascending tof order.
///
/// The centroider only looks at a sliding window of `tof_kernel_len + 1` tof
/// indices and advances one at a time, so the full transpose -- one hash map
/// per digitizer sample, ~640k per frame -- never needs to exist. Each scan's
/// ions are already ascending in tof, so this is a k-way merge over the scans.
#[derive(Debug)]
pub(crate) struct TofSlices<'a> {
    frame: &'a Frame,
    cursor: Vec<usize>,
    end: Vec<usize>,
...
impl<'a> TofSlices<'a> {
    pub(crate) fn new(frame: &'a Frame) -> Self {
        let offsets = frame.ions().scan_offsets();
        let n_scans = offsets.len().saturating_sub(1);
        let mut cursor = Vec::with_capacity(n_scans);
        let mut end = Vec::with_capacity(n_scans);
        let mut heap = std::collections::BinaryHeap::with_capacity(n_scans);
        let mut max_scan = 0u32;

        for (scan, w) in offsets.windows(2).enumerate() {
            cursor.push(w[0]);
            end.push(w[1]);
            if w[0] < w[1] {
                let tof = usize::from(frame.ions().tof_indices()[w[0]]) as u32;
                heap.push(std::cmp::Reverse((tof, scan as u32)));
                max_scan = max_scan.max(scan as u32);
            }
        }
    ...
    pub(crate) fn max_tof(&self) -> usize {
        self.max_tof
    }

    pub(crate) fn max_scan(&self) -> u32 {
        self.max_scan
    }

    /// Replace `out` with the `(scan, intensity)` pairs at `tof`.
    /// Must be called with non-decreasing `tof`.
    pub(crate) fn fill(&mut self, tof: usize, out: &mut Vec<(u32, u64)>) {
        out.clear();
        let tof = tof as u32;
        while let Some(&std::cmp::Reverse((t, scan))) = self.heap.peek() {
            if t > tof {
                break;
            }
            self.heap.pop();
            let i = self.cursor[scan as usize];
            if t == tof {
                out.push((scan, u64::from(self.frame.ions().intensities()[i])));
            }
            let next = i + 1;
            self.cursor[scan as usize] = next;
            if next < self.end[scan as usize] {
                let nt = usize::from(self.frame.ions().tof_indices()[next]) as u32;
                self.heap.push(std::cmp::Reverse((nt, scan)));
            }
        }
    }
}
...
#[cfg(test)]
pub(crate) fn frame_from_ions(
    n_scans: usize,
    ions: &[(usize, u32, u32)],
) -> timsrust_core::Frame {
    use timsrust_core::{Frame, FrameIons};
    let mut by_scan: Vec<Vec<(u32, u32)>> = vec![Vec::new(); n_scans];
    for &(scan, tof, intensity) in ions {
        by_scan[scan].push((tof, intensity));
    }
    let mut scan_offsets = Vec::with_capacity(n_scans + 1);
    let mut tof_indices = Vec::new();
    let mut intensities = Vec::new();
    scan_offsets.push(0);
    for scan in by_scan.iter_mut() {
        scan.sort_unstable();
        for &(tof, intensity) in scan.iter() {
            tof_indices.push(tof.try_into().expect("tof out of range"));
            intensities.push(intensity.try_into().expect("intensity out of range"));
        }
        scan_offsets.push(tof_indices.len());
    }
    let mut frame = Frame::default();
    frame.set_ions(FrameIons::new(scan_offsets, tof_indices, intensities));
    frame
}
